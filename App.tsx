...
import { AppStatus, WorkerMessage, LlmProvider } from './types';
...
const OLLAMA_MODELS_CONFIG = [
  { id: 'qwen:0.5b', name: 'Qwen 0.5B' }, // Added Qwen 0.5B
...
const App = (): React.ReactNode => {
  ...
  const [selectedGenerationModelByProvider, setSelectedGenerationModelByProvider] = useState({
    ...
    ollama: GENERATION_MODELS_CONFIG.ollama[0].id, // Default to the first Ollama model
  });
  ...
  // Effect to fetch worker script and create Blob URL
  useEffect(() => {
    ...
    const createWorkerBlobUrl = async () => {
      ...
      try {
        const response = await fetch('./worker.js'); // Fetch from same origin
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    setEnvConfigs({
        ...
        ollamaModelName: process.env.OLLAMA_MODEL_NAME || GENERATION_MODELS_CONFIG.ollama[0].id // Use default from config
    });
  }, []);
  ...
  const getCurrentGenerationModel = useCallback(() => {
    switch (llmProvider) {
      ...
      case 'ollama':
        // Use selected model from dropdown, fallback to env var (which itself defaults to the first model)
  ...
  // Effect to initialize worker when Blob URL is ready and provider/keys/embedding model/generation model change
  useEffect(() => {
    if (!workerBlobUrl) {
    ...
    if (llmProvider === 'gemini' && !activeGeminiKey) {
      currentConfigError = "Gemini API Key not found. Please provide it via the input field or set API_KEY environment variable.";
    } else if (llmProvider === 'openai' && !activeOpenAIKey) {
      currentConfigError = "OpenAI API Key not found. Please provide it via the input field or set OPENAI_API_KEY environment variable.";
    } else if (llmProvider === 'ollama' && !envConfigs.ollamaBaseUrl) { 
      currentConfigError = "Ollama Base URL (OLLAMA_BASE_URL env var) not found.";
    }
    if (!currentGenModel) {
  ...
  // Dependencies for re-initializing worker
  ...
  const handleLlmProviderChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // Reset generation model for the new provider to its default
  ...
  const handleProcessDocument = useCallback(async () => {
    ...
    if (selectedFiles.length > 0) {
      try {
        ...
        setDocumentText(combinedText); // Update textarea for consistency if files were primary input
        handleClearFiles(); // Clear selected files after processing
      } catch (error: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-700 p-4 md:p-8 flex flex-col items-center text-gray-200">
        ...
        {llmProvider === 'ollama' && (
          <div className="p-3 bg-slate-800 rounded-lg">
            ...
            <p className="mt-2 text-xs text-slate-400">
              ...
              Default Ollama URL: `{envConfigs.ollamaBaseUrl}`. Configure via `OLLAMA_BASE_URL` environment variable if needed.
            </p>
          </div>
        )}
      </div>
      ...
      {llmProvider === 'gemini' && (
        <div className="w-full max-w-4xl mb-4 p-3 bg-slate-800 rounded-lg">
          <label htmlFor="geminiApiKey" className="block text-sm font-medium text-slate-300 mb-1">Gemini API Key (Optional - uses env var if blank):</label>
          <input
            type="password"
            id="geminiApiKey"
      ...
      {llmProvider === 'openai' && (
         <div className="w-full max-w-4xl mb-4 p-3 bg-slate-800 rounded-lg">
          <label htmlFor="openAIApiKey" className="block text-sm font-medium text-slate-300 mb-1">OpenAI API Key (Optional - uses env var if blank):</label>
          <input
            type="password"
            id="openAIApiKey"
      ...
      { (llmProvider === 'gemini' || llmProvider === 'openai') && (
        <p className="w-full max-w-4xl text-xs text-slate-400 mb-4 text-center">
            Note: API keys entered here are used in your browser and not stored persistently by this app, but be mindful of browser extensions or other local factors. Using environment variables (if your deployment supports them) is generally more secure for keys.
        </p>
      )}
